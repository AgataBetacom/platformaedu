...
function finishSelection(){document.getElementById('nav-catalog').classList.remove('bg-primary','text-white','shadow-md');document.getElementById('nav-catalog').classList.add('hover:bg-white/10','text-slate-300');switchTab('dashboard');}
function togglePackageBook(btn,cardId){const card=document.getElementById(cardId);const isSelected=btn.classList.contains('selected-pkg');if(!isSelected){if(selectedBooks>=5){showLimitToast();return;}
btn.classList.add('selected-pkg','bg-primary/10','text-primary','border-primary/30');btn.classList.remove('bg-slate-50','text-slate-500','border-slate-100');btn.innerHTML='✓ W pakiecie';card.classList.add('book-in-package');selectedBooks++;}else{btn.classList.remove('selected-pkg','bg-primary/10','text-primary','border-primary/30');btn.classList.add('bg-slate-50','text-slate-500','border-slate-100');btn.innerHTML='Wybierz +';card.classList.remove('book-in-package');selectedBooks--;}
updatePackageBar();}
function updatePackageBar(){const display=document.getElementById('counter-display');const hint=document.getElementById('counter-hint');if(display){display.textContent=String(selectedBooks);if(selectedBooks===5){if(hint)hint.textContent='Wykorzystano cały pakiet';}else if(selectedBooks>0){if(hint)hint.textContent='Możesz jeszcze wybrać '+(5-selectedBooks)+(5-selectedBooks===1?' książkę':' książek');}else{if(hint)hint.textContent='Możesz wybrać do 5 książek';}}
for(let i=1;i<=5;i++){const bar=document.getElementById('bar-'+i);if(bar){bar.className='cf-progress-seg'+(i<=selectedBooks?' filled':'');}}
const confirmBtn=document.getElementById('confirm-btn');if(confirmBtn){if(selectedBooks>=1){confirmBtn.disabled=false;confirmBtn.className='cf-cta';}else{confirmBtn.disabled=true;confirmBtn.className='cf-cta';}}}
function showLimitToast(){const toast=document.createElement('div');toast.className='fixed top-6 left-1/2 -translate-x-1/2 z-50 bg-navy text-white px-6 py-3 rounded-2xl font-bold text-sm shadow-2xl';toast.innerHTML='⚠️ Pakiet pozwala wybrać maksymalnie 5 książek';document.body.appendChild(toast);setTimeout(()=>toast.remove(),2500);}
